...
app.service('LoginService', function ($http, $q) {
	var self = {
		...
		getLoginResult: function getLoginResult() {
			var d = $q.defer();
			$http.jsonp('http://io.papayaplay.com/auth/user.do?callback=JSON_CALLBACK').success(function (res) {
...
app.service('EventService', function ($http, $q, LoginService) {
	var self = {
		...
		setJoinCountry: function setJoinCountry(unid, code) {
			$http({
				method: 'jsonp',
				url: 'http://io.papayaplay.com/secured/json.do?callback=JSON_CALLBACK',
				params: { cm: 'Service.Event.SetUser_BSCS80', unid: unid, countrycode: code }
		...
		getEventUserEvent: function getEventUserEvent(unid) {
			var d = $q.defer();
			$http({
				method: 'jsonp',
				url: 'http://io.papayaplay.com/json.do?callback=JSON_CALLBACK',
				params: { cm: 'Service.Event.GetEventResult_BSCS80', unid: unid }
			}).success(function (res) {
